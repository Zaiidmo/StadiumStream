...
class buyticketModel extends Connection
{
    public function fetchSingleMatche($id)
    {
        try {
            ...
            m.id,T1.name AS team1,T2.name AS team2,S.name as stadiumName,DATE(m.time) AS matchDay, T1.flag AS flag1, T2.flag AS flag2, m.ticket_price AS price FROM `match` m 
