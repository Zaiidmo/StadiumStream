    ...
    const validationOptions = [
        {
            ...
            errMessage: (input, label) => `${label.textContent} needs to be at least ${input.minLength} characters`
        },
        {
            attribute: 'costommaxlength',
            isValid: input => input.value && input.value.length <= parseInt(input.getAttribute("costommaxlength"), 10),
            errMessage: (input, label) => `${label.textContent} needs to be less than ${input.getAttribute("costommaxlength")} characters`
        },
        {
            attribute: 'pattern',
            isValid: input => {
